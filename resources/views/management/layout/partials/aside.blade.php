<aside class="sidenav-main nav-expanded nav-lock nav-collapsible sidenav-light navbar-full sidenav-active-rounded">
    ...
    <ul class="sidenav sidenav-collapsible leftside-navigation collapsible sidenav-fixed menu-shadow" id="slide-out"
        ...
        @if($user->hasRole('content') || 1)
            <li class="bold"><a class="waves-effect waves-cyan " href="{{ route('course.list') }}"><i
                        class="material-icons">format_list_bulleted</i><span class="menu-title" data-i18n="Chat">درس های
                        من</span></a>
            </li>
        @endif
        </li>
        <li class="bold"><a class="waves-effect waves-cyan " href="{{ route('publics') }}"><i
                    class="material-icons">dvr</i><span class="menu-title" data-i18n="Contacts">دوره های
                    ملیسان</span></a>
        </li>

        <li class="bold"><a class="waves-effect waves-cyan " href="{{ route('konkors') }}"><i
                    class="material-icons dp48">event_available</i><span class="menu-title" data-i18n="Calendar">آزمون
                    ها</span></a>
        </li>
        </li>
        @if($user->hasRole('content'))

            <li class="bold"><a class="waves-effect waves-cyan " href="/dashboard/survey/cats"><i
                        class="material-icons">import_contacts</i><span class="menu-title" data-i18n="File Manager"> نظر
                        سنجی</span></a>
            </li>
        @endif
        ...
        @php
            $user=\Illuminate\Support\Facades\Auth::user();
              $content=\App\Models\Coworker::where('user_id',$user->id)->first();
            @endphp
        ...
        <li class="bold"><a class="waves-effect waves-cyan " href="{{ route('baroms') }}"><i
                    class="material-icons">border_all</i><span class="menu-title" data-i18n="File Manager"> بارم
                    بندی</span></a>
        </li>
        ...
        @endif
        @php
                $mosabeghat=\App\Models\Touradmin::where('user_id',$user->id)->count();
            @endphp
        ...
        @if($user->hasRole('teacher'))
            @php
                        $user2=\App\Models\User::where('national',\Illuminate\Support\Facades\Auth::user()->national)->where('role',2)->first();
                    @endphp
            @if($user2)
        ...
        @if($user->hasRole('student'))
            @php
                        $user2=\App\Models\User::where('national',\Illuminate\Support\Facades\Auth::user()->national)->where('role',1)->first();
                    @endphp
            @if($user2)
                <li class="bold"><a class="waves-effect waves-cyan " href="{{ route('change') }}"><i
                            class="material-icons dp48">exit_to_app</i><span class="menu-title" data-i18n="File Manager"> انتقال
